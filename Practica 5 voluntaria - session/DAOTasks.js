    ...
    getAllTasks(email, callback) {
        this.pool.getConnection(function (err, connection) {
            if (err) {
                ...
                connection.query("SELECT task.id, task.text, task.done, tag.tag FROM user JOIN (task LEFT JOIN tag ON task.id = tag.taskId) ON user.email = task.user WHERE user.email=?",
                    [email],
                    function (err, rows) {
                ...
                connection.query("INSERT INTO task(user,text,done) VALUES(?,?,?)",
                    ...
                    function (err, rows) {
                        ...
                        if (err) {
                            ...
                            else if(task.tagsFin.length>0){
                                let array = []
                                console.log(task.tagsFin)

                                task.tagsFin.forEach(e => {
